...
class PosZReport(models.TransientModel):
	...
	def generate_z_report(self):
		...
		return self.env.ref('reporte_cierre_de_caja.action_z_report_print').report_action([], data=data)

class ClosedSessionReport(models.AbstractModel):

	_name = 'report.reporte_cierre_de_caja.report_closed_session'
	...
	def get_sale_details(self,sessions=False,company=False):
		...
		for order in orders:
			...
			for line in order.lines:
				...
				category = []
				for cat in line.product_id.reporte_cierre_de_caja_catrgory:
